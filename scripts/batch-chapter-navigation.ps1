# 批量处理脚本：为第三章到第七章的所有小节页面添加导航功能
# 目标：统一为所有小节页面添加章节导航功能

$baseDir = "src/pages/book1"

# 定义所有需要处理的小节文件（第三章到第七章，排除7.2已完成的）
$chapterFiles = @(
    # 第三章
...
Write-Host "开始批量处理章节导航功能..." -ForegroundColor Green
        ...
        try {
            ...
            if ($content -notmatch "function scrollToSection") {
                ...
                if ($content -match '## 📋 章节概览') {
                    # 为overview-item添加onclick属性（需要根据实际章节数量调整）
                    # 这里先添加基本的导航脚本和样式
...
Write-Host "`n批量处理完成！" -ForegroundColor Green
