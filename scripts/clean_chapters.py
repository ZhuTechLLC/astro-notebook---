...
def clean_content_thoroughly(content):
    """彻底清理内容"""
    lines = content.split('\n')
    cleaned_lines = []
    ...
    for line in lines:
        ...
        if any(marker in line for marker in [
            ':::tip 核心要点', ':::gpt-tip', '<div class="core-points">', 
            '<div class="nav-links">', '</div>', '<!-- 图表占位'
        ]):
            skip_blocks = True
            continue
            
        if skip_blocks and line.strip() == ':::' or line.strip() == '</div>':
            skip_blocks = False
            continue
        ...
        # 跳过重复的导航链接
        if any(nav in line for nav in ['← 上一章', '下一章 →', '返回目录', '导航链接']):
            skip_nav = True
            continue
            
        if skip_nav and line.strip() == '':
            skip_nav = False
            continue
...
def process_chapter_file(file_path, chapter_order):
    ...
    # 彻底清理现有内容
    content = clean_content_thoroughly(content)
