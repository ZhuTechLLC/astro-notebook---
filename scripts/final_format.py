...
def clean_content_selectively(content):
    """选择性清理内容，只移除重复的导航和gpt-tip块"""
    lines = content.split('\n')
    cleaned_lines = []
    
    # 移除gpt-tip块和旧的core-points块
    skip_blocks = False
    for line in lines:
        # 跳过gpt-tip块
        if ':::gpt-tip' in line:
        ...
        # 跳过旧的core-points块
        if '<div class="core-points">' in line:
            skip_blocks = True
            continue
        if skip_blocks and line.strip() == '</div>':
            skip_blocks = False
            continue
        if skip_blocks:
            continue
...
def process_chapter_file(file_path, chapter_order):
    ...
    # 选择性清理现有内容
    content = clean_content_selectively(content)
